...
function BeforeAfter({ before, after, className = '' }) {
  ...
  return (
    <div ref={containerRef} className={`relative select-none rounded-2xl overflow-hidden bg-black ${className}`}>
      <img src={before} alt="before" decoding="async" className="absolute inset-0 w-full h-full object-contain" draggable={false} />
      <div className="absolute inset-0 overflow-hidden will-change-[clip-path]" style={{ clipPath: `inset(0 ${100 - pct}% 0 0)` }}>
        <img src={after} alt="after" decoding="async" className="absolute inset-0 w-full h-full object-contain" draggable={false} />
      </div>
      <div className="absolute top-0 bottom-0 z-10 pointer-events-none" style={{ left: `${pct}%` }}>
        <div className="w-px h-full bg-white/70 shadow-[0_0_10px_rgba(255,255,255,0.55)]" />
      </div>
      <div
      ...
      <div className="absolute top-3 left-3 text-[10px] uppercase tracking-widest text-white/85 bg-black/45 px-2 py-1 rounded-full">Before</div>
