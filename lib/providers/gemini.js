...
// IMPORTANT: instantiate the client lazily on first use so that a cold module
// import never blows up when NODE has not yet materialized `.env` values.
// (The OpenAI SDK throws "OPENAI_API_KEY missing" if constructed with a
// falsy apiKey — which we saw happen during hot reloads.)
let _client = null
function getClient() {
  if (_client) return _client
  const apiKey = process.env.EMERGENT_LLM_KEY || process.env.OPENAI_API_KEY
  if (!apiKey) {
    throw new Error('EMERGENT_LLM_KEY is not configured on the server. Please set it in /app/.env and restart Next.js.')
  }
  _client = new OpenAI({
    apiKey,
    baseURL: 'https://integrations.emergentagent.com/llm',
    // Keep timeouts generous — image generation can take 20-40s.
    timeout: 90_000,
    maxRetries: 1,
  })
  return _client
...
export async function generateFashionImage({ prompt, productImg, modelImg, backgroundImg }) {
  ...
  // Emergent gateway returns image data either in msg.images[] (b64_json) or
  // as data URLs inside content. Handle both.
